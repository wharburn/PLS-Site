...
const AILegalAdvice: React.FC<AILegalAdviceProps> = ({ lang = 'en' }) => {
  const t = translations[lang].aiLegal;
  ...
  const categories = [
    { id: t.propertyLaw, icon: '🏠', bg: 'bg-amber-50' },
    { id: t.immigrationLaw, icon: '🛂', bg: 'bg-blue-50' },
    { id: t.familyLaw, icon: '👨‍👩‍👧', bg: 'bg-rose-50' },
  ];
    ...
    <div className="max-w-7xl mx-auto px-6">
      <div className="flex flex-col lg:flex-row gap-16 items-start">
        <div className="lg:w-1/2">
          ...
          <h2 className="text-4xl font-bold text-slate-900 mb-6 font-serif">{t.title}</h2>
          <p className="text-slate-600 text-lg mb-8 leading-relaxed">{t.desc}</p>
            ...
            {categories.map((cat) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <span
                    className={`w-12 h-12 flex items-center justify-center rounded-lg text-2xl ${cat.bg}`}
                  >
                    {cat.icon}
                  </span>
                ...
                {savedConsultations.map((save) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button
                        onClick={(e) => deleteSaved(save.id, e)}
                        ...
                        {save.sources && save.sources.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {save.sources.slice(0, 3).map((s, i) => (
                              <div
                                key={i}
                                className="text-[9px] text-indigo-500 bg-indigo-50 px-2 py-1 rounded truncate max-w-[150px]"
                              >
                                {s.title}
                              </div>
                            ))}
            ...
            <div className="flex-grow flex flex-col">
              <div className="flex items-center justify-between mb-6 pb-2 border-b border-slate-50">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Grounding Analysis Output
                </span>
                {response && (
                  <div className="flex gap-4">
                    <button
                      onClick={handleSave}
                      className="text-[10px] font-bold text-slate-400 hover:text-amber-500 transition-colors uppercase tracking-widest"
                    >
                      {saveSuccess ? 'Saved to Vault' : 'Save Session'}
                    </button>
                    <button
                      onClick={() => handleCopy(response)}
                      className="text-[10px] font-bold text-slate-400 hover:text-indigo-500 transition-colors uppercase tracking-widest"
                    >
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-2">
                    ...
                    {sources.length > 0 && (
                      <div className="mt-8 pt-6 border-t border-slate-100">
                        <span className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-4 block">
                          Verified Legal Footnotes
                        </span>
                          ...
                          {sources.map((s, idx) => (
                            ...
                            >
                              ...
                              <div className="flex-grow min-w-0">
                                <div className="text-[10px] font-bold text-slate-800 truncate group-hover:text-amber-900 transition-colors">
                                  {s.title || 'Official Document'}
                                </div>
                                <div className="text-[9px] text-slate-400 truncate opacity-60">
