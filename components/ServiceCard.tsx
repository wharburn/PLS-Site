...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, lang }) => {
  ...
  const tc = translations[lang].serviceCards;
  ...
  return (
    ...
    >
      <div className="aspect-[4/3] overflow-hidden relative">
        <img
          ...
          className={`w-full h-full object-cover transition-transform duration-1000 ${
            isExpanded ? 'scale-110 blur-[2px]' : 'group-hover:scale-110'
          }`}
        />
        ...
        <div
          className={`absolute top-4 right-4 bg-amber-500 text-white text-[9px] font-black uppercase px-2 py-1 rounded transition-all duration-300 ${
      ...
      <div className="p-8 flex-grow flex flex-col relative bg-white z-10">
          ...
          <p
            className={`text-slate-600 leading-relaxed transition-all duration-500 ${
              isExpanded
                ...
                : 'opacity-100 translate-y-0 line-clamp-2'
            }`}
          >
            {service.description}
          </p>
        ...
        <div className="mt-auto pt-6 flex items-center justify-between">
          ...
          <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            {isExpanded ? tc.pressExit : tc.clickMore}
          </span>
        </div>
      </div>
      ...
      <div
        className={`absolute bottom-0 left-0 h-1 bg-amber-500 transition-all duration-700 ease-in-out ${
          isExpanded ? 'w-full' : 'group-hover:w-full'
        }`}
