...
const AdminInvoicesPage: React.FC = () => {
  ...
  return (
    <AdminShell title="Invoices" subtitle="From Supabase table: invoices (schema-driven render until you confirm columns)">
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        />
        <div style={{ fontSize: 12, color: '#64748b', fontWeight: 900, alignSelf: 'center' }}>{loading ? 'Loading…' : `${filtered.length} invoices`}</div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td colSpan={Math.max(columns.length, 1)} style={{ padding: 16, color: '#64748b', fontWeight: 800 }}>No invoices found.</td>
              </tr>
            )}
          </tbody>
        </table>
