...
export const getLegalAdvice = async (query: string, category: string, language: string = 'en') => {
  ...
  const response = await ai.models.generateContent({
    ...
    Query: ${query}`,
    config: {
      systemInstruction: "You are the AI Legal Assistant for PLS Consultants. You are knowledgeable about UK and Portuguese law. Be professional, cautious, and always recommend a consultation with a specialist. Reference search sources clearly.",
      temperature: 0.3,
      tools: [{ googleSearch: {} }]
    }
  });
...
export const chatWithNoVo = async (message: string, history: { role: 'user' | 'model', content: string }[], language: string = 'en') => {
  if (!API_KEY) return "System Offline: No API Key.";

  const langPrompt = language === 'pt' ? 'Ensure you respond in Portuguese to the user.' : 'Ensure you respond in English to the user.';
...
export const translateDocument = async (
  ...
  Extract text and translate from ${sourceLang} to ${targetLang}. 
  Return ONLY the translated text. Terminology: Legal/Business.
  Input: ${sourceText}`;
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [{ parts }],
    config: {
      systemInstruction: "You are a CIOL-certified professional translator.",
      temperature: 0.1,
    }
  });
...
export const analyzeImage = async (imageBase64: string, userPrompt: string, language: string = 'en') => {
  if (!API_KEY) return "Service Unavailable.";

  const langPrompt = language === 'pt' ? 'Provide analysis in Portuguese.' : 'Provide analysis in English.';
  const prompt = `Analyze this image for PLS Consultants. ${langPrompt} Inquiry: ${userPrompt || 'General analysis.'}`;
